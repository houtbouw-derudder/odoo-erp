...
#     atom_expr = Group(param + oper + (numeric | textual) | lpar + param +
#                       oper + (numeric | textual) + rpar).setParseAction(_parse_expr)
#     expr = Forward()
#     expr <<= (atom_expr + ZeroOrMore(logical_op + expr)) | (lpar + atom_expr +
#                                                             ZeroOrMore(logical_op + expr) + rpar).setParseAction(_parse_grouped_expr)
...
#         l, op, r = result[:3]
#         _result = [_op_funcs[op](
#             evaluate(l, vals), evaluate(r, vals))] + result[3:]
#         if len(_result) > 1:
...
#     else:
#         raise RuntimeError(
#             "Unsupported data type {0} {1}".format(result, type(result)))
...
#             try:
#                 parsed = parse(
#                     "dries < 600 and (bram > 900 or (fien = 'ja' or broes = 'nee') and fien < 'zus')")
